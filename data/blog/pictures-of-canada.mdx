...
- ^For efficient youtube embed resulting in least page load :
```html
<YouTube video="lh8dNmneVyY"/>
```

- ^For Custom videos and manifests `(use one only)` :
```html
<Spark src="<YOUR_VIDEO_SOURCE>" poster="<YOUR_VIDEO_POSTER>" />

<Clappr src="<YOUR_VIDEO_SOURCE>" poster="<YOUR_VIDEO_POSTER>" />
```
